...
static void BM_1000_Orders(benchmark::State& state) {
...
static void BM_10000_Orders(benchmark::State& state) {
    // Setup (once per benchmark, not per iteration)
    auto engine = std::make_shared<MatchingEngine>();
    auto gateway = std::make_unique<OrderGateway>(engine);
    auto generator = OrderGenerator{2};

    std::array<OrderGateway::OrderRequest, 10000> orders;
    for (size_t i = 0; i < orders.size(); i++) {
        orders[i] = generator.generate_order();
    }
...
static void BM_100000_Orders(benchmark::State& state) {
    ...
    std::array<OrderGateway::OrderRequest, 100000> orders;
    for (size_t i = 0; i < orders.size(); i++) {
        orders[i] = generator.generate_order();
    }
...
BENCHMARK(BM_10000_Orders);
BENCHMARK(BM_100000_Orders);
